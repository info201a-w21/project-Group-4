...
summary_info <- list(emergency_food_by_city)
summary_info$num_observations <- nrow(emergency_food_by_city)
summary_info$some_max_value <- emergency_food_by_city %>%
  filter(num_food_sources) == max(num_food_sources, na.rm = T) %>%
  select(num_food_sources) 
